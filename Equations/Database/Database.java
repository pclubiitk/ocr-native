...
public class Database {
	...
	public static int GetAllCharacter()
	{
		...
		int[] Black_vertical=new int[Width];
		...
		for(int i=0; i<Height; i++)
		{
			...
			//	Text[LineNo]=merge.combine(Text[LineNo],special_char,text_coordinates,special_text_coordinates,original);
				int left=0,right=0;	
				ImageLib.FrequencyOfBlack_vertical(binarized_image,low,high,Black_vertical);
				current_line="";
				for(int k=0;k<Width;k++)
				{
					//System.out.println(Black_vertical[k]);	
					if(Black_vertical[k] != 0)
					{
						left = k;	
						while(Black_vertical[k] != 0)
							k++;
						right = k-1;
					
					coordinates[0][0] = left;
					coordinates[0][1] = right;
					//System.out.println(coordinates[0][1]);
					ImageLib.GetFrame(binarized_image,coordinates,low,high);
					...
					for(int x=coordinates[0][0]; x<=coordinates[0][1]; x++)
					{
						for(int y=coordinates[1][0]; y<=coordinates[1][1]; y++)
						{
							...
							character_image.setRGB(x-coordinates[0][0],y-coordinates[1][0],newpixel);
						}
					}
